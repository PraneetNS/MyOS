...
typedef enum { PROC_UNUSED = 0, PROC_READY, PROC_EXITED } proc_state_t;
...
/* Sets up the fixed process table with slot 0 as the always-resident
   shell (ring0, kernel address space, entry = shell_run). Call once,
...
/* Loads an ELF into a fresh process slot + fresh address space and
   marks it READY to run. Returns NULL if no free slot or the load
   failed (out of memory, malformed ELF, etc). Does NOT start running
   it -- the scheduler picks it up on its next switch. */
